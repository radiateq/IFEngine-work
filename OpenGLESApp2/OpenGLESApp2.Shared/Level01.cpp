...
namespace Level01{
 ...
 void CPongLevel::PostOperations() {
 ...
 void CPongLevel::AdvanceGame() {
  if (game_engine->EGL_initialized) {
   ...
   if (!DEMO4_initialized) {
    ...
    radius = game_engine->width > game_engine->height ? game_engine->width : game_engine->height, dummy = 0.0;
    radius *= 0.02;
    radius += game_engine->width*0.5;
    Window2ObjectCoordinates(radius, dummy, zDefaultLayer, game_engine->width, game_engine->height);
    ...
    //zoom_factor = game_engine->width < game_engine->height ? game_engine->width : game_engine->height, dummy = 0.0;
    //zoom_factor /= 12.0 * 24.0;
    //zoom_factor += game_engine->width*0.5;
    //Window2ObjectCoordinates(zoom_factor, dummy, zDefaultLayer, game_engine->width, game_engine->height);
     ...
     if (IFGameEditor::GetTouchEvent()) {
      ...
      //if (temp_int64 > 0) 
      {
       input_event_time_stamp = IFGameEditor::GetTouchEvent(&touchx, &touchy);
       {
        ...
        Window2ObjectCoordinates(screenx, screeny, zDefaultLayer, game_engine->width, game_engine->height);
 ...
 void CPongLevel::Init_IFAdapter() {
  if (game_engine->EGL_initialized) {
 ...
 void CPongLevel::Setup_OpenGL(double width, double height) {
